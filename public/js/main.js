...
$_document.on("DOMContentLoaded", setBaseCssOnLoad());

// DOM is ready
function setBaseCssOnLoad() {
  // Document variables
  var $_body = $("body"),
    ...
    if (
      ...
      $_product_configurator_input.each(function(input_key) {
        var $_configurator_input = $(this),
          ...
          configurator_input_data_title_plural = $_configurator_input.data(
            "title-plural"
          ),
          configurator_input_data_description = $_configurator_input.data(
            "description"
          );
            ...
            if ($_product_configurator_sync.length) {
              $_product_configurator_sync.each(function() {
                var $_product_configurator_sync_item = $(this),
                  $_product_configurator_sync_item_total = $_product_configurator_sync_item.find(
                    "div.configurator-total span.value"
                  ),
                  $_product_configurator_sync_item_input = $_product_configurator_sync_item.find(
                    "div.configurator-row input"
                  ),
                  $_product_configurator_sync_item_input_active = $_product_configurator_sync_item_input.filter(
                    '[name="' + $_configurator_input.attr("name") + '"]'
                  ),
                  $_product_configurator_sync_item_input_active_row = $_product_configurator_sync_item_input_active.parents(
                ...
                update_configurator({
                  el_configurator_title: $_product_configurator_sync_item_input_active_row.find(
                  ...
                  val_configurator_title: $_product_configurator_sync_item_input_active.data(
                    "title"
                  ),
                  val_configurator_title_plural: $_product_configurator_sync_item_input_active.data(
                    "title-plural"
                  ),
                  val_product_configurator_currency: $_product_configurator_sync_item.data(
                    "currency"
                  ),
                });
              });
  ...
  $("div.content-background").each(function() {
    ...
    if ($_content_background_item.length) {
      var content_background_icon = $_content_background_item.is("i"),
        content_background_media = $_content_background_item.is(
          "svg, img, video"
        ),
    ...
    if ((location_hash = this.hash)) {
      ...
      if (
        ...
        if (($_location_target = $(location_hash)).length) {
          ...
          $_html_body.animate(
            {
              scrollTop: (location_target_scroll_position = location_scroll_position()),
            },
            Math.max(
              400,
              Math.round(
